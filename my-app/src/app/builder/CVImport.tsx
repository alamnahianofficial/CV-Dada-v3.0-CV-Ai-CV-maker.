"use client";
// ─── CV IMPORT — Upload old CV (PDF/DOCX) → auto-fill form ───────────────────
...
// ─── Extract text from DOCX client-side (no server needed) ───────────────────
...
// ─── Extract text from PDF via server route (avoids pdfjs-dist bundle) ────────
...
export default function CVImport({ setResume }: Props) {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      setErrorMsg("Could not read file. Make sure it's a valid PDF or DOCX.");
      setStatus("error");
      return;
    }
    ...
    if (!rawText.trim()) {
      setErrorMsg(
        "File appears empty or is a scanned image. Try a text-based PDF.",
      );
      setStatus("error");
      return;
    }
    ...
    await parseCVText(rawText, setResume, (s) => {
      if (s === "done") setStatus("done");
        ...
        setErrorMsg("AI could not parse your CV. Try again or fill manually.");
        setStatus("error");
      }
    });
  };
  ...
  return (
    <div className="sec-box" style={{ borderColor: "rgba(99,102,241,0.25)" }}>
      ...
      >
        Upload your old PDF or DOCX — AI will auto-fill all fields instantly.
      </p>
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 10, color: "#475569", margin: "2px 0 0" }}>
                All fields filled. Review and edit as needed.
              </p>
            </div>
          </div>
          <button
